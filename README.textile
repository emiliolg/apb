...
* Simple extraction (Option _-x_): Extracts everything under a subdirectory of the current working directory.
* Installation under specified _bin_ and _lib_ directories: Just pass the _bin_ directory as an argument, you can also use the _--lib_ option to specify a directory where to copy all the jars. Additionally the _--ant_ option can be used to indicate a directory where to install the jar with the ant tasks for *apb*.
